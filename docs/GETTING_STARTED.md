...
The header shows the active **data source** (`demo`, `github`, or `gitlab`) and connected repository when configured.
...
4. Loads optional **`.reviewgraph.json`** for ADRs and forbidden dependencies ([example](reviewgraph-config.example.json)).
...
## 8. Generate the presentation decks

```bash
npm run decks
```

Produces three real `.pptx` files in `decks/dist/`:

- `01-business-overview.pptx` — the problem, the idea, features, business value, positioning.
- `02-technical-overview.pptx` — the three layers, schema, enterprise workflow, feature traversals, the Aura agent.
- `03-getting-started.pptx` — this guide as slides, plus the roadmap.

Slides are generated from the **live** code-review pack, so they never drift from the code.
...
- **Port 4000 in use** — another server is running; stop it (or set `PORT`).
- **UI shows "Neo4j fallback"** — expected without `.env`; the app uses the in-memory graph. Set credentials and `npm run seed` to go live.
